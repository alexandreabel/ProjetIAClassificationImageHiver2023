...
public class AffichageNombreObjetActivity extends AppCompatActivity {
    ...
    LinearLayout linearLayout7up, linearLayoutAlexandreAbel, linearLayoutNesquikChocolat, linearLayoutNesquikFraise, linearLayoutOignonEspagnol, linearLayoutChocolatSkor, linearLayoutCoffeeCrisp, linearLayoutDeodorantAxe, linearLayoutMoutarde, linearLayoutOrange, linearLayoutPomme, linearLayoutSauceHotChicken, linearLayoutThonRioMare;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        linearLayoutAlexandreAbel = findViewById(R.id.linearLayoutAlexandreAbel);  //sert a cahché les linearLayout si la valeur est 0
        linearLayoutNesquikChocolat = findViewById(R.id.linearLayoutNesquikChocolat);  //sert a cahché les linearLayout si la valeur est 0
        linearLayoutNesquikFraise = findViewById(R.id.linearLayoutNesquikFraise);  //sert a cahché les linearLayout si la valeur est 0
        linearLayoutOignonEspagnol = findViewById(R.id.linearLayoutOignonEspagnol);  //sert a cahché les linearLayout si la valeur est 0
        linearLayoutChocolatSkor = findViewById(R.id.linearLayoutChocolatSkor);  //sert a cahché les linearLayout si la valeur est 0
        linearLayoutCoffeeCrisp = findViewById(R.id.linearLayoutCoffeeCrisp);  //sert a cahché les linearLayout si la valeur est 0
        linearLayoutDeodorantAxe = findViewById(R.id.linearLayoutDeodorantAxe);  //sert a cahché les linearLayout si la valeur est 0
        linearLayoutMoutarde = findViewById(R.id.linearLayoutMoutarde);  //sert a cahché les linearLayout si la valeur est 0
        linearLayoutOrange = findViewById(R.id.linearLayoutOrange);  //sert a cahché les linearLayout si la valeur est 0
        linearLayoutPomme = findViewById(R.id.linearLayoutPomme);  //sert a cahché les linearLayout si la valeur est 0
        linearLayoutSauceHotChicken = findViewById(R.id.linearLayoutSauceHotChicken);  //sert a cahché les linearLayout si la valeur est 0
        linearLayoutThonRioMare = findViewById(R.id.linearLayoutThonRioMare);  //sert a cahché les linearLayout si la valeur est 0
       ...
       tbPrix7up.setText(String.valueOf(new DecimalFormat("#0.00").format(prixTotal7up))+" $");
        ...
        miseAJoursPrixTotalDeToutLesElement();  //met la varialbe qui contient le totalt de totu les element a jours
...
/*
        ...
        if(nombreAlexandreAbel <= 0){
            linearLayoutAlexandreAbel.setVisibility(View.GONE);
        }  //desativer temporairement

        if(nombreNesquikAuChocolat <= 0){
            linearLayoutNesquikChocolat.setVisibility(View.GONE);
        }  //desativer temporairement

        if(nombreNesquikAuFraise <= 0){
            linearLayoutNesquikFraise.setVisibility(View.GONE);
        }  //desativer temporairement

        if(nombreOignonEspagnol <= 0){
            linearLayoutOignonEspagnol.setVisibility(View.GONE);
        }  //desativer temporairement

        if(nombreChocolatSkor <= 0){
            linearLayoutChocolatSkor.setVisibility(View.GONE);
        }  //desativer temporairement

        if(nombreCoffeeCrisp <= 0){
            linearLayoutCoffeeCrisp.setVisibility(View.GONE);
        }  //desativer temporairement

        if(nombreDeodorantAxe <= 0){
            linearLayoutDeodorantAxe.setVisibility(View.GONE);
        }  //desativer temporairement

        if(nombreMoutarde <= 0){
            linearLayoutMoutarde.setVisibility(View.GONE);
        }  //desativer temporairement

        if(nombreOrange <= 0){
            linearLayoutOrange.setVisibility(View.GONE);
        }  //desativer temporairement

        if(nombrePomme <= 0){
            linearLayoutPomme.setVisibility(View.GONE);
        }  //desativer temporairement

        if(nombreSauceHotChicken <= 0){
            linearLayoutSauceHotChicken.setVisibility(View.GONE);
        }  //desativer temporairement

        if(nombreThonRioMare <= 0){
            linearLayoutThonRioMare.setVisibility(View.GONE);
        }  //desativer temporairement
